...
<script src='https://api.mapbox.com/mapbox.js/v2.4.0/mapbox.js'></script>
	...
	<script>
		function changekor_halte() 
		{
		    ...
		    window.location="{{ base_url() }}manajemen_halte/"+selectedValue;
		}
		function changekor_point() 
		{
		    ...
		    console.log(selectedValue);
		    window.location="{{ base_url() }}manajemen_point/"+selectedValue;
		}

		function delete_point($id)
		{	
			var selectedValue = $id;
			window.location="{{ base_url() }}delete_point/"+selectedValue;
		}
		function delete_halte($id)
		{	
			var selectedValue = $id;
			window.location="{{ base_url() }}delete_halte/"+selectedValue;
		}
		...
		{	
			swal({   title: "Apakah Anda Yakin Akan Menghapus?",   text: "Koridor Yang Dipilih Akan Terhapus",   type: "warning",   showCancelButton: true, cancelButtonText: "Batalkan",   confirmButtonColor: "#DD6B55",   confirmButtonText: "Ya, Hapus!",   closeOnConfirm: false }, 
				function(){   
					...
					var selectedValue = $id;
					window.location="{{ base_url() }}delete_koridor/"+selectedValue;
				}
			);
		...
		{	
			var selectedValue = $id;
			window.location="{{ base_url() }}delete_rute/"+selectedValue;
		}
		function add_point()
		{	
			// var selectedValue = $id;
			window.location="{{ base_url() }}tambah_point";
		}
		function add_halte()
		{	
			// var selectedValue = $id;
			window.location="{{ base_url() }}tambah_halte";
		}
		function add_koridor()
		{	
			// var selectedValue = $id;
			window.location="{{ base_url() }}tambah_koridor";
		}
		function add_rute()
		{	
			// var selectedValue = $id;
			window.location="{{ base_url() }}tambah_rute";
		}
		function edit_point($id)
		{	
			var selectedValue = $id;
			window.location="{{ base_url() }}edit_point/"+selectedValue;
		}
		function edit_halte($id)
		{	
			var selectedValue = $id;
			window.location="{{ base_url() }}edit_halte/"+selectedValue;
		}
		function edit_koridor($id)
		{	
			var selectedValue = $id;
			window.location="{{ base_url() }}edit_koridor/"+selectedValue;
		}
		function edit_rute($id)
		{	
			var selectedValue = $id;
			window.location="{{ base_url() }}edit_rute/"+selectedValue;
		}
		var oTable1 = 
				$('#dynamic-table')
