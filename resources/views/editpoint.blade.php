...
@section('content-tabel')
	<form class="form-horizontal" role="form" method="post" action="<?php echo $point_terpilih->id?>">
		<input type="hidden" name="_token" value="{{ csrf_token() }}">
		<div class="form-group">
			<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Nomor </label>
			<div class="col-sm-9">
				<input type="text" name="Nomor" placeholder="Nomor Urutan Point" class="col-xs-10 col-sm-5"  value="<?php echo $point_terpilih->nomor?>"/>
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Longitude </label>
			<div class="col-sm-9">
				<input type="text" name="Longitude" placeholder="Longitude" class="col-xs-10 col-sm-5" value="<?php echo $point_terpilih->longitude?>" />
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> Latitude </label>
			<div class="col-sm-9">
				<input type="text" name="Latitude" placeholder="Latitude" class="col-xs-10 col-sm-5" value="<?php echo $point_terpilih->latitude?>" />
			</div>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Koridor Id</label>
				...
				<?php foreach ($koridor as $key => $value){ ?>
					<?php if ($point_terpilih->koridor_id==$value->id) { ?>
		...
		<div class="form-group">
			<label class="col-sm-3 control-label no-padding-right" for="form-field-1">Relasi</label>
				...
				<?php foreach ($halte as $key => $value){ ?>
					<?php if ($point_terpilih->halte_id==$value->id) { ?>
					<option selected value="<?php echo $value->id?>"><?php echo $value->id?> : <?php echo $value->nama?></option>
					<?php } else{ ?>
					<option value="<?php echo $value->id?>"><?php echo $value->id?> : <?php echo $value->nama?></option>
					<?php }} ?>
